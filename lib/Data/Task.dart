...
class Task {
  ...
  factory Task.fromFirestore(DocumentSnapshot doc) {
    ...
    return Task(
      ...
      acceptedAt: acceptedAt != null ? (acceptedAt as Timestamp).toDate() : null,
      startedAt: startedAt != null ? (startedAt as Timestamp).toDate() : null,
      completedAt: completedAt != null ? (completedAt as Timestamp).toDate() : null,
      done: done,
      status: data['status'] ?? 'pending',
      comments: data['comments'] ?? '', // ✅ Include comments here
    );
  }
  ...
  Task({
    ...
    this.id = '',
    this.status = 'pending',
    this.comments = '', // ✅ Also include here
  });
}
